...
(function (TokenType) {
    TokenType[TokenType["Constant"] = 0] = "Constant";
    TokenType[TokenType["Symbol"] = 1] = "Symbol";
    TokenType[TokenType["Operator"] = 2] = "Operator";
    TokenType[TokenType["Whitespace"] = 3] = "Whitespace";
...
export function parse(tokens) {
    tokens = tokens.filter((token) => token.type !== TokenType.Whitespace && token.type !== TokenType.Comment);
    let position = 0;
    function syntaxError() {
        console.log({ tokens, position });
        return new Error(`Syntax Error!`);
    }
    function match(toMatch, at = position) {
